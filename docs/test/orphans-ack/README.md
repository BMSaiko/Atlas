...
> Test file: `test/orphans-ack.test.mjs` (213 lines)  
> Helper: `_atlas-runtime.mjs` (spinAtlas — Vite middleware + atlas-api in-process)  
> Source under test: inline / multiple

## Purpose
Cobre /api/w/:slug/orphans/ack (POST) e o enriquecimento do GET /orphans (card h1y3yfsy: heartbeat + classification + logTail + orphanWorktreePath). Estilo: vanilla node:assert + spinAtlas. SOURCE EQUALITY (api.ts:1061+).

## How to run
node test/orphans-ack.test.mjs
...
## Source / invariant
See the header comment at the top of `test/orphans-ack.test.mjs` for exact file:line references. Many tests end with a "SOURCE EQUALITY" block that grep-checks production code for specific strings — if production drifts silently, the test fails.
