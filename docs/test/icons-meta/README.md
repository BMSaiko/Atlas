...
> Test file: `test/icons-meta.test.mjs` (79 lines)  
> Helper: `_atlas-runtime.mjs` (spinAtlas — Vite middleware + atlas-api in-process)  
> Source under test: inline / multiple

## Purpose
Cobre /api/icons e /api/w/:slug (meta.json GET). Estilo: vanilla node:assert. SOURCE EQUALITY (api.ts:807, 1346-1348).

## How to run
node test/icons-meta.test.mjs
...
## Source / invariant
See the header comment at the top of `test/icons-meta.test.mjs` for exact file:line references. Many tests end with a "SOURCE EQUALITY" block that grep-checks production code for specific strings — if production drifts silently, the test fails.
