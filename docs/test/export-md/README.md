...
> Test file: `test/export-md.test.mjs` (186 lines)  
> Helper: `_atlas-runtime.mjs` (spinAtlas — Vite middleware + atlas-api in-process)  
> Source under test: inline / multiple

## Purpose
Cobre /api/w/:slug/export: exporta notas nao-arquivadas para markdown em <VAULT>/knowledge/projects/<slug>/docs/notas.md. Ordena por ts asc. Formata tags (quoted se com espaco) e front-matter (id, criado ISO). Estilo: vanilla node:assert. SOURCE EQUALITY (api.ts:1225-1244).

## How to run
node test/export-md.test.mjs
...
## Source / invariant
See the header comment at the top of `test/export-md.test.mjs` for exact file:line references. Many tests end with a "SOURCE EQUALITY" block that grep-checks production code for specific strings — if production drifts silently, the test fails.
