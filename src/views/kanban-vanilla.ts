  ...
  function kops(c: Card): string {
    ...
    if (c.colId === 'todo') {
      ...
      if (!c.superPromptBody) {
        b.push(`<button class="btn-icon btn-ghost" data-act="generate-sp" data-card="${c.id}" aria-label="Gerar Super Prompt" data-cmd="kanban.gerar-sp">${icon('pencil', 15)}</button>${crashBadge}`)
      } else {
      ...
      b.push(`<button class="btn-icon btn-ghost" data-act="refine" data-card="${c.id}" aria-label="Refinar Super Prompt" data-cmd="kanban.refinar-sp">${icon('pencil', 15)}</button>`)
    }
    if (!b.length) return ''
  ...
  _pidPollTimer = setInterval(async () => {
    document.querySelectorAll<HTMLElement>('[data-pid-chip]').forEach(async (el) => {
      ...
      try {
        ...
        el.textContent = r.pid ? `agent: running (pid ${r.pid})` : 'agent: starting…'
      } catch { /* transient */ }
    })
  }, 5000)
}
