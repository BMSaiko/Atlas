...
export default function Workspace() {
  ...
  const [tab, setTab] = useState<Tab>(() => {
    ...
    if (qtab === 'notes' || qtab === 'kanban' || qtab === 'dash') return qtab
    try {
      const v = slug ? localStorage.getItem(`atlas.tab.${slug}`) : null
      if (v === 'notes' || v === 'kanban' || v === 'dash') return v as Tab
    } catch {}
    return 'dash'
  })
  ...
  useEffect(() => {
    ...
    else renderKanban(content, slug)
  ...
  useEffect(() => {
    if (!slug) return
    const TAB_ORDER: Tab[] = ['dash', 'notes', 'kanban']
